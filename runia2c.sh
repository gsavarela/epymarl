...
do
   for i in ${seeds[@]}
   do
      python src/main.py --config=ia2c --env-config=gymma with env_args.key="lbforaging:Foraging-$e" seed=$i &
      echo "Running with ia2c and lbforaging:Foraging-$e for seed=$i"
      sleep 2s
   done
   wait
done
