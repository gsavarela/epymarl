...
def main(
    ...
    # HOTFIX: Use this to keep the union
    # index = np.array(sorted(set.intersection(*map(set, map(lambda x: list(x.index), dataframes)))))
    ...
    # 3. Plots
    subdir = query_group if any(query_group) else 'ecai'
    print(subdir)
    task_plot(
        xs,
        mus,
...
if __name__ == "__main__":
    ...
    # ENV = 'mpe:SimpleSpread-v0'
    # ENV = 'mpe:SimpleAdversary-v0'
    # ENV = 'rware-tiny-4ag-v1'
    ENV = 'lbforaging:Foraging-2s-10x10-3p-3f-v1'
    # ENV = 'lbforaging:Foraging-15x15-3p-5f-v1'
    # ENV = 'lbforaging:Foraging-15x15-4p-5f-v1'
    # STRESS_TEST ENV
    # ENV = 'lbforaging:Foraging-2s-15x15-4p-5f-v1'
    # ENV = 'lbforaging:Foraging-2s-15x15-2p-3f-coop-v1'
    # ENV = 'lbforaging:Foraging-2s-15x15-2p-5f-coop-v1'
    # ENV = 'lbforaging:Foraging-15x15-2p-5f-coop-v1'
    # ENV = 'lbforaging:Foraging-2s-15x15-3p-5f-coop-v1'
    # ENV ='lbforaging:Foraging-10x10-3p-1f-coop-v1'
    # ENV = 'lbforaging:Foraging-2s-10x10-2p-3f-coop-v1'
    # algosnames = ('vdn',)
    # algosnames = ('ia2c_ns', 'ntwa2c',)
    # algosnames = ('ia2c_ns', 'ntwa2c', 'maa2c_ns')
    algosnames = ('ia2c', 'ntwa2c', 'maa2c')
    # algosnames = ('iql', 'ntwql', 'vdn')
    # algosnames = ('ia2c', 'maa2c')    # ntwa2c
    # algosnames = ('ia2c', 'ntwa2c', 'maa2c')
    # source = 'remote'
    # sources = [_q.pop('source') if 'source' in _q else 'remote' for _q in NTWQL_QUERIES[ENV].values()]

    main(ENV, algosnames, query_group='partial_observability')
