...
FIGURE_X = 6.0
FIGURE_Y = 4.0
CONSENSUS_PATTERN = r"critic_loss_mse_(\d+)"
...
def get_opt() -> Tuple[Path, Dict, Dict]:
    ...
    def fv(x):      # filter values
        return 'v_mean_batch_player' in x[0]
    values = dict(map(mwb, filter(fv, metrics.items())))

    def ft(x):      # filter values
        return 'v_mean_batch_target' in x[0]
    targets = dict(map(mwb, filter(ft, metrics.items())))

    def fl(x):      # filter loss
        return 'critic_loss' == x[0]
    loss = dict(map(mwb, filter(fl, metrics.items())))

    def fg(x):      # filter critic loss
        return 'critic_loss_mse' in x[0]
    reg = dict(map(mwb, filter(fg, metrics.items())))

    # info = {'weight': weight, 'task': task, 'step': step}
    info = {'task': task, 'step': step, 'model': model}
    # return weights, biases, values, info
    return loss, reg, values, targets, info

# TODO: place data within info dict
def plots(weights: Dict, biases: Dict, values: Dict, targets: Dict, info: Dict) -> None:
    """Plots consensus rounds """
    fig = plt.figure()
    fig.set_size_inches(FIGURE_X, FIGURE_Y)
    cr = info['consensus_rounds'] + 1
    pl = info['num_players'] + 1
    wid = info.get('weight', None)
    ...
    for z in ('v_mean_batch_player',):
        if z == 'v_mean_batch_player':
            data = values

        elif z == 'v_mean_batch_target':
            data = values
        else:
        ...
        for x in range(cr):
            for y in range(pl):
                ...
                if z == 'weight':
                    ...
                    key +='_0'
                dict_or_none = data.get(key, None)
                if dict_or_none is None: # missing values
                    Y[x, y] = None
                else:
                    Y[x, y] = data[key]['values']

        T = np.stack([targets[f'v_mean_batch_target_{i}']['values'] for i in range(cr)])
        ...
        plt.plot(X, Y.mean(axis=1), linestyle='dashed', label='approx.')
        plt.plot(X, T, linestyle='dashdot', label='target')
...
def main():
    ...
    # values, info = get_opt()
    loss, reg, values, targets, info = get_opt()
    ...
    info['consensus_rounds'] = max(map(mtchr, list(reg.keys())))
