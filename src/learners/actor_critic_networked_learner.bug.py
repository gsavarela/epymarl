        ...
        with th.no_grad():
            ...
            for k in range(self.consensus_rounds):
                ...
                for _key, _weights in consensus_parameters.items():
                    ...
                    _w = _weights[0].clone()
                    if 'weight' in _key:
                        _w1 = th.einsum('nm, mij-> nij', cwm, _w)
                    elif 'bias' in _key:
                        _w1 = th.einsum('nm, mi-> ni', cwm, _w)
                    else:
                        raise ValueError(f'Unknwon parameter type {_key}')

                    consensus_parameters[_key] = [_w1]
                    consensus_parameters_logs[_key + f'_{k + 1}'] = [_w1]
                ...
                np.testing.assert_almost_equal(self.critic.critics[0].fc1.bias.numpy(),
                    consensus_parameters['fc1.bias'][0][0, :].numpy())
                np.testing.assert_almost_equal(self.critic.critics[0].fc1.weight.numpy(),
                    consensus_parameters['fc1.weight'][0][0, :].numpy())
