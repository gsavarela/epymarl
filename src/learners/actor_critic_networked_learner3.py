...
class ActorCriticNetworkedLearner3:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        target_vals, critic_train_stats = self.consensus_step(batch, critic_mask)

        
        advantages = self.train_critic_sequential(
            self.critic, target_vals, batch, rewards, critic_mask, critic_train_stats
        )
    ...
    def train_critic_sequential(self, critic, target_vals, batch, rewards, mask, running_log):
        if self.args.standardise_returns:
    ...
    def consensus_step(self, batch, mask):
        ...
        with th.no_grad():
            ...
            vs = []
            for _i in range(self.n_agents):
                vs.append(self.target_critic(batch, _i))
            v_target = th.cat(vs, dim=2)

            # consolidates episode segregating by player
            v_target_mean_player = ((v_target[:, :t_max] * mask).sum(dim=(0, 1)) / mask.sum(dim=(0, 1))).numpy().round(6)
            for _i in range(self.n_agents):
