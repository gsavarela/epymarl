...
class CAdam(Optimizer):
    def __init__(self, local_params, consensus_params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
    ...
    def step(self, closure=None, *, grad_scaler=None):
        ...
        for lcl, cns, prv in zip(local, consensus, prev):
            ...
            if lcl.grad is not None:
                ...
                grads.append(lcl.grad)

                state = self.state[lcl]
                # Lazy state initialization
                if len(state) == 0:
                    state['step'] = (
                        torch.zeros((1,), dtype=torch.float, device=lcl.device)
                        if self.defaults['capturable'] or self.defaults['fused']
                        else torch.tensor(0.)
                    )
                    # Exponential moving average of gradient values
                    state['exp_avg'] = torch.zeros_like(lcl, memory_format=torch.preserve_format)
                    # Exponential moving average of squared gradient values
                    state['exp_avg_sq'] = torch.zeros_like(lcl, memory_format=torch.preserve_format)
                    if group['amsgrad']:
                        # Maintains max of all exp. moving avg. of sq. grad. values
                        state['max_exp_avg_sq'] = torch.zeros_like(lcl, memory_format=torch.preserve_format)
...
def cadam(params: List[Tensor],
         ...
         *,
         ...
         eps: float,
         maximize: bool):
    r"""Functional API that performs CAdam algorithm computation.
    See :class:`~torch.optim.CAdam` for details.
    """
...
def _single_tensor_adam(params: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        if capturable or differentiable:
            ...
            param.data =  torch.addcdiv(param.data, exp_avg, denom, value=-step_size)
